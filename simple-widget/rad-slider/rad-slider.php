 ...
 * use the template tag rad_slider() anywhere in your theme templates 
 * to show the slides
 */
function rad_slider(){
	...
	$slide_query = new WP_Query( array(
		'post_type' => 'slide',
		'showposts' => 5,
		'nopaging' => true,  //ignore pagination if displayed on an archive
	) );
...
function rad_slider_scripts(){
	...
	wp_register_style( 'rad-slider-style', $style_url );
	wp_enqueue_style( 'rad-slider-style' );

	//attach jquery (built into WordPress)
	wp_enqueue_script( 'jquery' );

	//attach responsiveslides script
	$rs_url = plugins_url( 'js/responsiveslides.js', __FILE__ );
	//(handle, src, dependency)
	wp_register_script( 'responsiveslides', $rs_url, 'jquery' );
	wp_enqueue_script('responsiveslides');

	//attach the JS file that activates the slider behavior
	$custom_url = plugins_url( 'js/custom.js', __FILE__ );
	wp_register_script( 'rs-custom', $custom_url, 'responsiveslides' );
